import { FileText } from 'lucide-react';

export function Propuestas() {
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">Propuestas</h1>
      <div className="rounded-2xl bg-zinc-900/50 border border-zinc-800 p-8 text-center">
        <FileText className="w-12 h-12 text-zinc-600 mx-auto mb-4" />
        <p className="text-zinc-400 mb-2">Tus propuestas aparecerán aquí.</p>
        <p className="text-sm text-zinc-500">Revisa, acepta o solicita cambios en las propuestas de tus proyectos.</p>
      </div>
    </div>
  );
}
